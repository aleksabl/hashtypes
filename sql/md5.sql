...
CREATE TYPE md5hash ( INPUT = md5_in, OUTPUT = md5_out, INTERNALLENGTH = 16);
...
CREATE FUNCTION text(md5hash) RETURNS TEXT IMMUTABLE LANGUAGE C STRICT AS 'hashtypes', 'md5_to_text';
CREATE CAST (md5hash AS text) WITH FUNCTION text(md5hash) AS ASSIGNMENT;

CREATE FUNCTION md5t(text) RETURNS md5hash IMMUTABLE LANGUAGE C STRICT AS 'hashtypes', 'text_to_md5';
CREATE CAST (text AS md5hash) WITH FUNCTION md5t(text) AS ASSIGNMENT;

CREATE FUNCTION md5b(bytea) RETURNS md5hash IMMUTABLE LANGUAGE C STRICT AS 'hashtypes', 'bytea_to_md5';
